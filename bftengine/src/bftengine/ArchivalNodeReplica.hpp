 ...
 *
 */
class ArchivalNodeReplica : public ReplicaForStateTransfer {
 public:
  ArchivalNodeReplica(const ReplicaConfig&,
 ...
 private:
  // This function serves as an ReplicaStatusHandlers alternative for ArchivalNodeReplica. The reason to use this
  // function is that regular and read-only replcias expose differen metrics and the status handlers are not
  // interchangable. The read-only replica also hasn't got an implementation for InternalMessages which are used by the
