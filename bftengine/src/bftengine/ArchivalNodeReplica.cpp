...
ArchivalNodeReplica::ArchivalNodeReplica(const ReplicaConfig &config,
  ...
  msgHandlers_->registerMsgHandler(
      MsgCode::Checkpoint, std::bind(&ArchivalNodeReplica::messageHandler<CheckpointMsg>, this, std::placeholders::_1));
  msgHandlers_->registerMsgHandler(
      MsgCode::ClientRequest,
      std::bind(&ArchivalNodeReplica::messageHandler<ClientRequestMsg>, this, std::placeholders::_1));
  msgHandlers_->registerMsgHandler(
      MsgCode::StateTransfer,
      std::bind(&ArchivalNodeReplica::messageHandler<StateTransferMsg>, this, std::placeholders::_1));
  metrics_.Register();
...
void ArchivalNodeReplica::start() {
...
void ArchivalNodeReplica::stop() {
  timers_.cancel(askForCheckpointMsgTimer_);
  ReplicaForStateTransfer::stop();
}

void ArchivalNodeReplica::onTransferringCompleteImp(uint64_t newStateCheckpoint) {
...
void ArchivalNodeReplica::onReportAboutInvalidMessage(MessageBase *msg, const char *reason) {
...
void ArchivalNodeReplica::sendAskForCheckpointMsg() {
...
template <>
void ArchivalNodeReplica::onMessage<StateTransferMsg>(StateTransferMsg *msg) {
  ReplicaForStateTransfer::onMessage(msg);
}

template <>
void ArchivalNodeReplica::onMessage<CheckpointMsg>(CheckpointMsg *msg) {
...
void ArchivalNodeReplica::persistCheckpointDescriptor(const SeqNum &seqnum, const CheckpointInfo<false> &chckpinfo) {
...
template <>
void ArchivalNodeReplica::onMessage<ClientRequestMsg>(ClientRequestMsg *m) {
...
void ArchivalNodeReplica::executeReadOnlyRequest(concordUtils::SpanWrapper &parent_span,
                                                 const ClientRequestMsg &request) {
...
void ArchivalNodeReplica::registerStatusHandlers() {
